#!/usr/bin/env python3
"""Exact payload verification for prepared and installed Python trees."""
...
def manifest(data):
    ...
    for line in data.decode().splitlines():
        ...
        parts = name.split("/")
        if name.startswith("/") or any(p.lower() == ".git" for p in parts):
        ...
        if (
            ...
            or "\\" in name
            or name.lower() in {p.lower() for p in result}
        ):
            raise ValueError(
                "Invalid or colliding payload manifest path: " + repr(name)
            )
...
def verify(directory, expected, allow_git=False):
    directory = Path(directory)
    ancestors = set()
    for name in expected:
        ancestors.update(str(p) for p in PurePosixPath(name).parents if str(p) != ".")
    actual, extras, bad = set(), [], []

    def walk(base, prefix=""):
        for entry in os.scandir(base):
            name = prefix + entry.name
            if name == ".git" and allow_git:
                continue
            try:
                info = entry.stat(follow_symlinks=False)
            except FileNotFoundError:
                raise ValueError(
                    "Filesystem changed during verification: " + repr(name)
                )
            ...
            if name in expected:
                ...
                if not stat.S_ISREG(info.st_mode):
                ...
                elif stat.S_IMODE(info.st_mode) != (
                    ...
                    with os.fdopen(fd, "rb") as stream:
                        ...
                        if (opened.st_dev, opened.st_ino, opened.st_mode) != (
                            ...
                            raise ValueError(
                                "Payload changed during verification: " + repr(name)
                            )
            ...
            if stat.S_ISDIR(info.st_mode):
                ...
                try:
                    ...
                    if (opened.st_dev, opened.st_ino) != (info.st_dev, info.st_ino):
                        raise ValueError(
                            "Directory changed during verification: " + repr(name)
                        )
                    walk(child, name + "/")
                finally:
                    os.close(child)
    ...
    summary = f"missing expected paths: {len(missing)}; unexpected paths: {len(extras)}; payload defects: {len(bad)}"
    if missing or extras or bad:
        ...
        raise ValueError(
            "Final payload verification failed; " + summary + "\n" + "\n".join(details)
        )
    return summary
    ...
    try:
        print(
            verify(
                ...
                len(sys.argv) > 3 and sys.argv[3] == "installed",
            )
        )
